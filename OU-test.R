...
{ 
        ...
        for (i in 1:N_time)
        {
                for (j in i:N_time)
                {
                        ...
                        if (i == j)     # Edward note: This is the Kroenker's delta - but why add epsilon square?
                        {
...
nonStochastic_two_compartments <- function(par, Y, t1){
        ...
        M <- t(Y_temp) %*% Y_temp;     # Note from Edward: cool way of doing sum of square.  But actually slower. 
